...
`conformance-check <repo>` runs eight checks and exits non-zero on any failure:
...
6. **Restore.** The manifest (`AGENT.md`) and identity are present and the log folds; run with `--fixture` against the fixture, the fold must additionally reproduce the four asserted graph facts. (Manifest-declared alternate locations and skills loading are restore obligations verified by check 8 and the runtime, respectively.)
...
8. **Identity-hash integrity.** The `identity_hash` pinned in `AGENT.md` must equal the SHA-256 over the `identity/` directory (or the legacy root `IDENTITY.md`), computed per the commit-rule-4 recipe — each file's relative path followed by a newline and its bytes, in byte-order sorted path order. A missing or unreadable manifest, a missing `identity_hash:` field, a missing `identity/` directory, or any hash mismatch fails closed. This closes the gap where the manifest declared an identity hash that nothing verified: a repo that re-pins after a legitimate human-gated identity edit (commit rule 4) stays conformant, while one whose `identity/` drifted or was tampered without re-pinning fails restore step 2.
